...
async def send_device_command(device_id: str, command: str, parameters: Optional[Dict] = None, ):
...
async def execute_scenario_async(scenario_id: str, db_pool):
    async with db_pool.acquire() as conn:
        ...
        await conn.execute("""
            ...
            VALUES ($1, $2, $3)
        """, execution_id, scenario_id, "running")

        # Обновляем время последнего выполнения
        await conn.execute(
            "UPDATE scenarios SET last_executed = $1 WHERE id = $2",
        ...
        for action in actions:
            try:
                ...
                await conn.execute("""
                    ...
                    VALUES ($1, $2, $3, $4, $5)
                """, action_id, execution_id, action["device_id"],
        ...
        await conn.execute("""
            ...
            WHERE id = $3
        """, "completed", datetime.utcnow(), execution_id)
