...
from warmhouse.ms_auth.services.auth import authenticate_user, create_access_token, get_current_user, get_user_by_email
from warmhouse.ms_auth.settings import ACCESS_TOKEN_EXPIRE_MINUTES, ALGORITHM, SECRET_KEY
from warmhouse.ms_auth.utils import get_password_hash, oauth2_scheme
...
async def login(
    ...
    return Token(
        access_token=access_token, token_type="bearer", expires_in=ACCESS_TOKEN_EXPIRE_MINUTES * 60, user_id=user["id"]
    )
...
async def register_user(
    ...
    return Token(
        access_token=access_token, token_type="bearer", expires_in=ACCESS_TOKEN_EXPIRE_MINUTES * 60, user_id=user_id
    )
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            session = await conn.fetchrow(
                "SELECT * FROM user_sessions WHERE token = $1 AND is_active = TRUE AND expires_at > NOW()", token
            )
            if not session:
                raise HTTPException(status_code=401, detail="Invalid or expired session")

        return {"valid": True, "user_id": user_id, "email": payload.get("email"), "expires_at": payload.get("exp")}
    except:
...
async def logout(pool=Depends(get_db), _: dict = Depends(get_current_user), token: str = Depends(oauth2_scheme)):
    """Выход пользователя"""
    async with pool.acquire() as conn:
        await conn.execute("UPDATE user_sessions SET is_active = FALSE WHERE token = $1", token)
